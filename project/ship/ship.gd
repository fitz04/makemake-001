...
@export var linear_acceleration := 50.0  # Increased for faster forward/backward (was 10.0)
...
@export var gravity_multiplier := 0.005  # Gravity coefficient (0.001-0.01)
...
func _integrate_forces(state: PhysicsDirectBodyState3D):
	...
	if stellar_body.type != StellarBody.TYPE_SUN:
		...
		var f := gravity_multiplier * stellar_mass / (gd * gd)
		f = minf(f, 25.0)
