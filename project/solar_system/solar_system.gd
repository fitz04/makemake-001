...
var _ship : Ship = null
var _player : Node3D = null  # Can be either ship or character
...
func _ready():
	...
	# Find Makemake planet index
	var makemake_id := -1
	for i in len(_bodies):
		if _bodies[i].name == "Makemake":
			makemake_id = i
			break

	# Set reference to Makemake before spawning ship
	if makemake_id != -1:
		set_reference_body(makemake_id)

	# Wait for terrain to be generated around spawn point before spawning ship
	if makemake_id != -1:
		var makemake := _bodies[makemake_id]
		var spawn_height := makemake.radius + 5.0  # 5 meters above surface
		var spawn_pos := Vector3(0, spawn_height, 0)

		progress_info.message = "Waiting for terrain generation..."
		loading_progressed.emit(progress_info)

		# Wait a bit for terrain generation to start
		# Just wait for a few frames instead of checking is_area_meshed
		# which seems to hang when the area is too large
		for i in range(120):
			await get_tree().process_frame

	# Carve craters (simplified: 1 do_sphere per crater, small radii)
	if makemake_id != -1:
		var makemake := _bodies[makemake_id]
		if makemake.crater_carver != null and makemake.volume != null:
			print("Carving craters on Makemake...")
			makemake.crater_carver.carve_all(makemake.volume)

	# Spawn ship on Makemake surface
	_ship = ShipScene.instantiate()
	if makemake_id != -1:
		var makemake := _bodies[makemake_id]
		# Position ship on the surface of Makemake (radius + offset for ship size)
		# Ship collision capsule: height 16m, radius 3m
		# With complex terrain (octaves=7, gain=0.5), rendering takes longer
		# Need more height to ensure terrain is generated before ship lands
		var spawn_height := makemake.radius + 150.0  # 100 meters above surface for safety
		var spawn_pos := Vector3(0, spawn_height, 0)
		print("Spawning ship at ", spawn_pos, spawn_height, " on Makemake surface")
		_ship.global_transform = Transform3D(Basis(), spawn_pos)
	else:
...
func _physics_process(delta: float):
	...
	if _player != null and _physics_count > 0 and _physics_count - _physics_count_on_last_reference_change > 10:
...
func _apply_ship_settings():
	...
	_player.gravity_multiplier = _settings.ship_gravity_multiplier
