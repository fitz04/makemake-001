...
## Apply changes to the generator and notify terrain to update
## This is expensive! Use carefully at runtime.
## @param generator: The VoxelGeneratorGraph that was modified
## @param voxel_terrain: Optional VoxelLodTerrain to notify for update
## @param modified_region: Optional AABB of the modified area (world space) to limit update
func apply_changes(generator: VoxelGeneratorGraph, voxel_terrain: VoxelLodTerrain = null, modified_region: AABB = AABB()) -> void:
	# 1. Recompile the graph
	# This is necessary for the changes to take effect in the generator
	var result = generator.compile()
	if result.success == false:
		push_error("CraterGenerator: Failed to compile graph after adding craters: " + result.message)
		return
	
	print("CraterGenerator: Graph recompiled successfully")

	# 2. Notify terrain to update
	if voxel_terrain != null:
		# If we have a specific region, we could try to only update that
		# But changing the graph structure usually requires a full reset/clear of the stream cache
		# because the SDF values have changed fundamentally.
		
		# For now, we'll trigger a full update which is safest but slowest
		# In a real game, you might want to use VoxelTool to modify voxels destructively
		# instead of modifying the generator graph at runtime, OR use a fixed graph with parameters.
		
		# Clearing the stream cache is important if the generator is saved/cached
		if voxel_terrain.stream != null:
			# If using SQLite or other persistent stream, we might need to invalidate the cache
			# For now, assuming procedural generation primarily
			pass
			
		# Resetting the terrain forces it to regenerate meshes from the generator
		# voxel_terrain.reset() # This might be too aggressive and cause visual blinking
		
		# Alternative: Invalidate specific area if possible, or just notify blocks
		# VoxelLodTerrain doesn't have a simple "reload all" without reset, 
		# but we can try to notify all blocks as modified? No, that saves them.
		
		# The standard way to refresh procedural generator changes is often just reset()
		print("CraterGenerator: Resetting terrain to apply changes...")
		voxel_terrain.generator = generator # Re-assigning might trigger update
		
		# Force update of meshes
		# Note: This might cause a lag spike
		# If you only want to update a specific area:
		# var tool = voxel_terrain.get_voxel_tool()
		# tool.set_mode(VoxelTool.MODE_ADD) # Dummy op?
		# tool.do_box(modified_region.position, modified_region.end) 
		# But that modifies data, we want to refresh from generator.
		
		# Best bet for "generator changed" is usually:
		voxel_terrain.notify_generator_changed()
