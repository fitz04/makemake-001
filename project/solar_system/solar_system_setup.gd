...
const CraterCarver = preload("./crater_carver.gd")
...
static func _setup_sun(body: StellarBody, root: Node3D) -> DirectionalLight3D:
	...
	# Kuiper Belt: sun at ~45 AU, very dim but still the brightest light source
	# Real brightness would be ~1/2000 of Earth, but we keep it playable
	directional_light.light_energy = 2.5  # Distant sun but playable brightness
	directional_light.light_color = Color(1.0, 0.95, 0.85)  # Slightly warm
	directional_light.shadow_opacity = 0.99
	directional_light.shadow_blur = 0.0  # Sharp shadows - no atmosphere to scatter
...
static func _setup_rocky_planet(body: StellarBody, root: Node3D, settings: Settings):
	...
	if body.name == "Mars":
		...
		# Ice dwarf planet - bright grayish-white surface with tholin
		mat.set_shader_parameter(&"u_top_modulate", Color(0.9, 0.95, 1.0))
		mat.set_shader_parameter(&"u_planet_radius", body.radius)

		# Biome centers on sphere surface (direction * radius)
		# Landing Basin: top of planet (spawn area)
		var landing_dir := Vector3(0, 1, 0).normalized()
		mat.set_shader_parameter(&"u_landing_basin_center", landing_dir * body.radius)
		mat.set_shader_parameter(&"u_landing_basin_radius", 1200.0)

		# Fire-Ice Basin: south pole region
		var fire_ice_dir := Vector3(0, -0.3, -1.0).normalized()
		mat.set_shader_parameter(&"u_fire_ice_center", fire_ice_dir * body.radius)
		mat.set_shader_parameter(&"u_fire_ice_radius", 1800.0)

		# Living Lode Canyon: far west
		var canyon_dir := Vector3(-1.0, 0.0, 0.2).normalized()
		mat.set_shader_parameter(&"u_canyon_center", canyon_dir * body.radius)
		mat.set_shader_parameter(&"u_canyon_radius", 2500.0)

		# Biome colors - Makemake surface (tholin + methane ice)
		mat.set_shader_parameter(&"u_landing_basin_color", Color(0.95, 0.90, 0.85))
		mat.set_shader_parameter(&"u_cryo_plains_color", Color(0.85, 0.75, 0.70))
		mat.set_shader_parameter(&"u_ore_highlands_color", Color(0.75, 0.60, 0.50))
		mat.set_shader_parameter(&"u_fire_ice_color", Color(0.65, 0.55, 0.50))
		mat.set_shader_parameter(&"u_canyon_color", Color(0.50, 0.40, 0.35))
	
	var generator : VoxelGeneratorGraph = BasePlanetVoxelGraph.duplicate(true)

	# Set up crater carver (craters are carved AFTER terrain generation via VoxelTool)
	# Keep craters small (radius < 200m) to avoid do_sphere() performance issues
	var carver := CraterCarver.new()
	carver.planet_radius = body.radius

	# Landing Basin crater (top of planet, at spawn) - medium
	carver.add_crater(Vector3(0, 1, 0), 150.0, 40.0, 0.0, "Landing Basin")
	# North crater
	carver.add_crater(Vector3(0.1, 0.7, 1.0), 120.0, 35.0, 0.0, "North")
	# South crater
	carver.add_crater(Vector3(-0.1, 0.3, -1.0), 100.0, 30.0, 0.0, "South")
	# East crater
	carver.add_crater(Vector3(1.0, 0.5, 0.2), 80.0, 25.0, 0.0, "East")
	# West crater (near Living Lode Canyon)
	carver.add_crater(Vector3(-1.0, 0.5, -0.1), 90.0, 25.0, 0.0, "West")
	# Northeast crater
	carver.add_crater(Vector3(1.0, 0.6, 1.0), 70.0, 20.0, 0.0, "Northeast")

	# Store carver on body for deferred carving (after terrain mesh generation)
	body.crater_carver = carver

	# Pass crater data to shader for dust rendering
	var crater_centers := PackedVector3Array()
	var crater_radii := PackedFloat32Array()
	for i in range(carver.get_craters().size()):
		crater_centers.append(carver.get_crater_surface_center(i))
		crater_radii.append(carver.get_craters()[i].radius)
	while crater_centers.size() < 10:
		crater_centers.append(Vector3.ZERO)
		crater_radii.append(0.0)
	mat.set_shader_parameter("u_crater_count", carver.get_craters().size())
	...
	# DEBUG: no other graph modifications — pure v4
	generator.compile()
...
static func setup_stellar_body(body: StellarBody, parent: Node, 
	...
	if body.type == StellarBody.TYPE_SUN:
		...
		# DEBUG: ice plains disabled for testing
		#if body.name == "Makemake":
		#	_setup_ice_plains(body, root)
...
## Create frozen ice plains that fill low-lying terrain areas.
## Uses a SphereMesh slightly below planet surface - where terrain noise dips below
## the ice level, flat frozen plains appear (methane, nitrogen, water ice mix).
static func _setup_ice_plains(body: StellarBody, root: Node3D) -> void:
	# Ice surface at radius - 40m: fills valleys deeper than 40m below mean surface
	# Place ice at -10m below mean surface. With height_mult -80, terrain ranges ~±80m,
	# so ice fills most valleys, creating large visible frozen plains.
	var ice_radius: float = body.radius - 10.0

	var ice_mesh := SphereMesh.new()
	ice_mesh.radius = ice_radius
	ice_mesh.height = 2.0 * ice_radius
	ice_mesh.radial_segments = 128
	ice_mesh.rings = 64

	var ice_material := StandardMaterial3D.new()
	# Frozen methane/nitrogen ice - slightly translucent, pale blue-white
	ice_material.albedo_color = Color(0.7, 0.78, 0.88, 1.0)  # Fully opaque ice
	ice_material.metallic = 0.15
	ice_material.roughness = 0.1  # Very smooth, glassy frozen surface
	ice_material.rim_enabled = true
	ice_material.rim = 0.4
	ice_material.rim_tint = 0.5

	var ice_instance := MeshInstance3D.new()
	ice_instance.name = "IcePlains"
	ice_instance.mesh = ice_mesh
	ice_instance.material_override = ice_material
	ice_instance.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF

	root.add_child(ice_instance)
	print("Added ice plains at radius ", ice_radius, " (", body.radius - ice_radius, "m below surface)")


## Add dust fog volumes to crater areas for atmospheric effect
static func _add_crater_dust_volumes(body: StellarBody, root: Node3D, carver) -> void:
	var craters = carver.get_craters()
	print("Adding dust fog volumes to ", craters.size(), " craters")

	for i in range(craters.size()):
		var crater = craters[i]
		var surface_center: Vector3 = carver.get_crater_surface_center(i)

		var fog_volume := FogVolume.new()
		fog_volume.name = "CraterDust_%d" % i
		fog_volume.size = Vector3(crater.radius * 2.2, 60.0, crater.radius * 2.2)
		...
		# Position on sphere surface + slight outward offset
		var up: Vector3 = crater.surface_direction
		fog_volume.position = surface_center + up * 30.0

		# Rotate fog volume to align with surface normal
		fog_volume.look_at_from_position(fog_volume.position, fog_volume.position + up, Vector3.UP)

		root.add_child(fog_volume)
		print("  Added dust volume for ", crater.label, " at ", surface_center)
